...
public class DiningPhilosopherProblem2 implements Runnable {
	...
	public static void main(String[] args){
		...
		DiningPhilosopherProblem2[] philosopher = new DiningPhilosopherProblem2[5];
		
		// start each running in its own thread
		for (int i = 0; i < 5; i++) {
			philosopher[i] = new DiningPhilosopherProblem2(String.valueOf(i), chopsticks[i], chopsticks[(i+1)%5]);
	...
	public DiningPhilosopherProblem2 (String name, Lock left, Lock right) {
		this.name = name;
		this.left = left;
		this.right = right;
	}
	...
	public void run() {
		try {
			while (true) {
				...
				putDown();
				if ("0".equals(name)) {
					DiningPhilosopherProblem2.philosopher0++;
				} else if ("1".equals(name)) {
					DiningPhilosopherProblem2.philosopher1++;
				} else if ("2".equals(name)) {
					DiningPhilosopherProblem2.philosopher2++;
				} else if ("3".equals(name)) {
					DiningPhilosopherProblem2.philosopher3++;
				} else if ("4".equals(name)) {
					DiningPhilosopherProblem2.philosopher4++;
				}
			}
